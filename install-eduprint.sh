...
if which lsb_release >/dev/null 2>/dev/null ; then
  ...
  DISTRO="CentOS"
elif cat /etc/system-release-cpe 2>/dev/null | grep fedora >/dev/null 2>/dev/null ; then
  DISTRO="Fedora"
else
...
case "$DISTRO" in
  ...
  *)
    ...
    echo " "
    echo "If your distribution uses either yum apt-get or dnf"
    ...
    echo "Looking at this script might be useful."
    echo " "
    echo "Enter y for yum, a for apt-get, d for dnf or o for other."
    read ANSWER
    case "$ANSWER" in
      y)
        PKG_MAN="yum"
...
case $PKG_MAN in
  ...
  "dnf" )
    if ! sudo dnf -y -q install samba-client foomatic ; then
      echo "At least one of the dependencies was not installed"
      echo "Please install manually. Looking at this script might be useful."
      exit 5
    fi
